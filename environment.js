const {Flags} = require("./const_values")
const {To2Dec, calcNormalVec} = require("./utils");
...
class Environment {
    ...
    parseObjInfo(objInfo) {
        ...
        if (cmd.p[0] === "p") {
            if (cmd.p.length > 1) {
                ...
                objInfo.type = "player"
                objInfo.ally = objInfo.team === this.agent.team
            }
    ...
    processObjs(objs) {
        ...
        minError = null
        for (let flag of flags) {
            let zeroVec = this.rotate(calcNormalVec(this, flag), flag.direction)
            let error = 0

            for (let flag2 of flags) {
                let fVec = calcNormalVec(this, flag2);
        ...
        for (let o of objs) {
            ...
            let vec = this.rotate(this.zeroVec, -o.direction)
